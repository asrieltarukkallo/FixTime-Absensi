...
<title>Absensi Masuk - MY-Visite</title>
        ...
        <div class="camera-card">
            <form id="formAbsen" method="POST" action="proses_absen_kamera.php">
                ...
                <button type="submit" id="btnSubmit" class="btn-submit" disabled>
                    <i class="bi bi-check-circle-fill me-2"></i>Submit Absensi Masuk
                </button>
            </form>
        </div>
    <?php endif; ?>
...
document.getElementById('formAbsen').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            ...
            btnSubmit.disabled = false;
            btnSubmit.innerHTML = '<i class="bi bi-check-circle-fill me-2"></i>Submit Absensi Masuk';
        }
    })
    .catch(error => {
